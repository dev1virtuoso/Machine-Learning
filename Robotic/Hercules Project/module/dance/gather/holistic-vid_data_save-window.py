...
def estimate_depth(frame, model, transform):
    # Ensure input dimensions are divisible by 32 to avoid size mismatches
    height, width = frame.shape[:2]
    if height % 32 != 0 or width % 32 != 0:
        new_height = (height // 32) * 32
        new_width = (width // 32) * 32
        print(f"Resizing frame from ({height}, {width}) to ({new_height}, {new_width}) for depth estimation...")
    ...
    # Perform depth estimation with debug checks
    with torch.no_grad():
        try:
            depth_map = model(input_batch)
        except RuntimeError as e:
            print(f"RuntimeError during depth estimation: {e}")
            return None  # Skip this frame if an error occurs
...
# Function to resize tensors safely
def resize_tensor(tensor, target_shape):
    resized_tensor = cv2.resize(tensor, target_shape[::-1], interpolation=cv2.INTER_LINEAR)
...
def extract_landmarks_with_depth(results, depth_map, image_shape, frame_count):
    ...
    if depth_map.shape[:2] != (image_shape[0], image_shape[1]):
        print(f"Resizing depth map from {depth_map.shape} to {image_shape[:2]}...")
        depth_map = resize_tensor(depth_map, (image_shape[1], image_shape[0]))
    ...
    if results.pose_landmarks:
        frame_data["landmarks"]["pose"] = {
            ...
            if i < len(pose_tubuh)  # Ensure index aligns with predefined pose landmarks
        }
        has_landmarks = True
